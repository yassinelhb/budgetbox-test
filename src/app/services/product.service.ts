...
export class ProductService {
  ...
  getProducts(size : number, from : number, includes : string, keywords : string, order : string) {
    return this.http.get(`api/products/v1.0/?size=${size}&from=${from}&includes=${includes}&keywords=${keywords}&order=${order}`);
  }
